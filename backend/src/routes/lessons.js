...
const handleGetAllLessons = async (c) => {
  try {
    ...
    if (tagsParam.length > 0) {
      ...
      whereClauses.push(
        `l.id IN (SELECT tl.lessonId FROM tagsonlessons tl JOIN tags t ON tl.tagId = t.id WHERE t.name IN (${tagPlaceholders}))`,
      );
    ...
    const selectSql = `
      ...
      LIMIT ? OFFSET ?
    `;
    const lessonsRow = await c.env.DB.prepare(selectSql)
    ...
    for (const name of tags) {
      ...
      if (trimmed) {
        await c.env.DB.prepare("INSERT OR IGNORE INTO tags (name) VALUES (?)")
          .bind(trimmed)
          .run();
        const tag = await c.env.DB.prepare("SELECT * FROM tags WHERE name = ?")
          .bind(trimmed)
          .first();
...
lessonRoutes.get("/stats", async (c) => {
  try {
    const totalLessons = await c.env.DB.prepare(
      "SELECT COUNT(*) as total FROM lessons WHERE isPublished = 1",
    ).first();
    const totalViews = await c.env.DB.prepare(
      "SELECT SUM(viewsCount) as total FROM lessons",
    ).first();
    ...
    return c.json(
      { totalBlogs: 3, totalLessons: 3, totalViews: 1500, totalTags: 5 },
      200,
    );
  }
});
...
const handleGetSingleLesson = async (c) => {
  try {
    ...
    const lesson = await c.env.DB.prepare(lessonSql)
      .bind(isNumeric ? parseInt(param) : param)
      .first();
    ...
    const slidesRow = await c.env.DB.prepare(slidesQuery)
      .bind(...bindParams)
      .all();
    ...
    const totalSlidesCount =
      totalSlidesInDb || lessonWithTags.slidesCount || slides.length;
  ...
  try {
      ...
      processedSlides = [
        {
          orderNumber: 1,
          title: "Introduction",
          blocks: [{ type: "paragraph", content: "Lesson content goes here." }],
        },
      ];
    }
    ...
    for (let i = 0; i < processedSlides.length; i++) {
      ...
      const blocksJson = JSON.stringify(
        slide.blocks || [{ type: "paragraph", content: slide.content || "" }],
      );
...
lessonRoutes.put("/edit/:id", authenticateUser, async (c) => {
  try {
    ...
    const body = await c.req.json();
    const { title, content, slides, imageUrl, excerpt, tagIds, isPublished } =
      body;
    ...
    const publishedVal =
      isPublished !== undefined ? (isPublished ? 1 : 0) : existing.isPublished;
    ...
    if (processedSlides.length > 0) {
      await c.env.DB.prepare("DELETE FROM slides WHERE lessonId = ?")
        .bind(id)
        .run();
      for (let i = 0; i < processedSlides.length; i++) {
        const slide = processedSlides[i];
        ...
        await c.env.DB.prepare(
          ...
          .bind(id, orderNum, slide.title || `Slide ${orderNum}`, blocksJson)
          .run();
      }
    }

    if (tagIds && Array.isArray(tagIds)) {
      await c.env.DB.prepare("DELETE FROM tagsonlessons WHERE lessonId = ?")
        .bind(id)
        .run();
      for (const tagId of tagIds) {
        await c.env.DB.prepare(
...
lessonRoutes.put("/reorder-slides/:id", authenticateUser, async (c) => {
  try {
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      if (slide.id) {
        await c.env.DB.prepare(
          "UPDATE slides SET orderNumber = ? WHERE id = ? AND lessonId = ?",
        )
    ...
    return c.json(
      { error: "Failed to reorder slides", details: error.message },
      500,
    );
  }
});
